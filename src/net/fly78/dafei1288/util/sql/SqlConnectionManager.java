...
import net.fly78.dafei1288.util.io.ConfigResources;

import com.mchange.v2.c3p0.*;

public class SqlConnectionManager {
	private static ComboPooledDataSource cpds = null;
	private static ConfigResources systemConfig = null;
	private static SqlConnectionManager sqlm = null;
	
	public static SqlConnectionManager getInstance(String file){
		if(sqlm ==null || systemConfig==null){
			systemConfig = new ConfigResources(file);
			sqlm = new SqlConnectionManager();
		}
		return sqlm;
	}
	...
	public ConfigResources getConfig(){
		return systemConfig;
		
	}
	
	public void init() {
		System.out.println("-----------sql  manager  init-------------");
		// 建立数据库连接池
...
/*
 * public class SqlConnection { private static SqlConnection dc; private
 * Connection conn = null; private Statement stmt = null;
 * 
 * private SqlConnection() { }
 * 
 * public static SqlConnection instance() { if (dc == null) { dc = new
 * SqlConnection(); } return dc; }
 * 
 * public Statement openStmt() { if (stmt == null) { conn = getConn(); try {
 * stmt = conn.createStatement(); } catch (Exception e) {
 * System.err.println("创建Statement异常:" + e.getMessage()); } } return stmt; }
 * 
 * public void closeStmt() { if (stmt != null) { try { stmt.close(); } catch
 * (Exception e) { System.err.println("Statement关闭异常"); } } if (conn != null) {
 * try { conn.close(); } catch (Exception e) { System.err.println("数据库关闭异常"); } } }
 * 
 * private Connection getConn() { if (conn == null) { try { // 创建连接池实例
 * ComboPooledDataSource ds = new ComboPooledDataSource(); // 设置连接池连接数据库所需的驱动
 * ds.setDriverClass("com.mysql.jdbc.Driver"); // 连接数据库的用户名; ds.setUser("root"); //
 * 连接数据库的密码 ds.setPassword("321478"); // 设置连接池的最大连接数 ds.setMaxPoolSize(40); //
 * 连接池的最小连接数 ds.setMinPoolSize(2); // 设置连接池的缓存Statement的最大数
 * ds.setMaxStatements(180); // 获得连接 conn = ds.getConnection(); } catch
